...
# ============================================================================
# 5. MULTI-SCALE LOSS with Cluster Contrastive Learning
# ============================================================================

class SymFormerLoss(nn.Module):
    """
    Combined loss function:
    1. Multi-scale segmentation loss
    2. Cluster contrastive loss (from kMaXU)
    3. Symmetry loss (asymmetry penalty)
    """
    def __init__(self, num_classes=2, class_weights=None):
        ...
        self.dice = DiceLoss(include_background=True, to_onehot_y=True, softmax=True)
        self.ce = nn.CrossEntropyLoss(weight=class_weights)
        
        # Loss weights
        self.seg_weight = 1.0
        self.cluster_weight = 0.1
        self.symmetry_weight = 0.05
        
    def forward(self, output, targets, cluster_outputs, asymmetry_map):
        """
        Args:
            output: (B, C, H, W) - final prediction
            targets: (B, H, W)
            cluster_outputs: List[(B, C, H_i, W_i)]
            asymmetry_map: (B, 1, D, H, W)
        """
        # 1. Main segmentation loss
        if targets.ndim == 3:
            targets_dice = targets.unsqueeze(1)
        else:
            targets_dice = targets
            
        main_loss = self.dice(output, targets_dice) + self.ce(output, targets.long())
        
        # 2. Multi-scale cluster loss
        cluster_loss = 0
        for i, cluster_out in enumerate(cluster_outputs):
            # Downsample target to match cluster output size
            scale = cluster_out.shape[-1] / targets.shape[-1]
            ...
            cluster_loss += self.dice(cluster_out, target_scaled.unsqueeze(1))
        
        cluster_loss /= len(cluster_outputs)
        
        # 3. Symmetry loss (penalize asymmetry in non-stroke regions)
        # Only penalize asymmetry where mask = 0 (background)
        healthy_mask = (targets == 0).float().unsqueeze(1).unsqueeze(2)
        healthy_mask = F.interpolate(healthy_mask, size=asymmetry_map.shape[2:], mode='nearest')
        
        symmetry_loss = (asymmetry_map * healthy_mask).mean()
        
        total_loss = (
            self.seg_weight * main_loss +
            self.cluster_weight * cluster_loss +
            self.symmetry_weight * symmetry_loss
        )
        
        return total_loss, {
            'seg_loss': main_loss.item(),
